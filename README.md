...
# 本次导入了5621条数据，目前数据库里的车次总数是4354
```
这是正常导入。

下面则是被12306反爬的情况。遇到这种情况，说明此次爬取结果无效，最好先不要保存，过一段时间再试。其实我感觉反爬机制挺迷的。有时候爬了十几条Y字头就被反爬了，有时候连续爬几千条G/D/C都不会被打断。
...
在测试程序的时候你也可以读取我已经导入的数据。我已经导入了2025年1月18日、1月22日和2月5日三天的全部车次，可以用于体验。
...
* 排序方式不同：在按站点查询中，本工具总是按开点计（终到的按到点计），可能存在不同（如路路通时刻表按到点排序）。
